...
### Source
> Extraído de `goal.md`, `tech-design.md` y `grill-me.md` en `/home/bryan/Downloads/Repos/crafter-repo`.
>
> "Construir un agente de codificación AI modelo-agnóstico, en una aplicación de escritorio basada en Tauri 2 (backend en Rust + frontend web en Svelte 5), que funcione en cualquier distribución de Linux y que sea súper ligero: debe correr fluidamente en hardware antiguo y en los dispositivos más modestos, sin importar que no sean antiguos."
...
| R0.1 | 🟡 Sin runtime VDOM pesado (Svelte 5 o Vanilla JS compilado). | Must-have |
...
### Forma A: Stack Oficial (Tauri 2 + Core Rust + Svelte 5)
...
| **A2** | **Frontend Svelte 5**: Compilado a JS vanilla sin runtime VDOM, asegurando consumo <500MB RAM y latencia UI < 50ms. | |
...
| Req | Requirement | Status | Forma A (Svelte 5) | Forma B (Preact) | Forma C (Daemon CLI) |
|-----|-------------|--------|:------------------:|:----------------:|:--------------------:|
...
1. **Slice V1 (MVP Chat & Multi-provider)**: Shell Tauri 2 + Svelte 5 + Adaptadores SSE (OpenAI, Anthropic, Gemini, Ollama) + Chat Streaming.
